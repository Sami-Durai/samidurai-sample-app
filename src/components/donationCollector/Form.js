...
const Form = ({ initialValue: { initValue: propInitValue, isEditable }, dataTableRef }) => {
  ...
  const addUpdateItem = useCallback(async (data) => {
    if (!isEditable)
      await response.add({
        service: service,
        method: "addDonationCollector",
        data: { item: data },
        toasterMessage: {
          success: `Donation Collector "${data.name}" has been created successfully`,
      ...
      await response.update({
        service: service,
        method: "updateDonationCollector",
        data: { itemId: initValue.id, item: data },
        toasterMessage: {
          success: `Donation Collector "${data.name}" has been updated successfully`,
