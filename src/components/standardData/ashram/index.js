...
// components
import Form from "components/standardData/ashram/Form";
...
const Ashram = () => {
    ...
    const setFormInitValue = useCallback(() => {
        ...
        modalPopup.custom({ header: "Add Ashram", className: "sdm-popup", visible: true });
    }, []);
    // add section end
    ...
    const editItem = useCallback((ev, rowData) => {
        ...
        modalPopup.custom({ header: "Update Ashram", className: "sdm-popup", visible: true });
    }, []);
    // update section start
    ...
    const removeItem = useCallback(async (id, name) => {
        await response.remove({
            ...
            data: { itemId: id },
            toasterMessage: {
                success: "Ashram" + (name ? ` "${name}"` : "") + " has been removed successfully",
                error: "Unable to remove ashram" + (name ? ` "${name}"` : "")
            },
            dataTable: tableRef
        });
    }, []);
            ...
            selection: {
                ...
                updateBtnsOptions: {
                    onClick: ({ selections, status }) => {
                        confirmDialog.custom({
                            message: "You are about to mass update the status of ashrams?",
