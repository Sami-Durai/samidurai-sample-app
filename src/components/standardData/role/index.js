...
// components
import Form from "components/standardData/role/Form";
...
const Role = () => {
    ...
    const setFormInitValue = useCallback(() => {
        ...
        modalPopup.custom({ header: "Add Role", className: "sdm-popup", visible: true });
    }, []);
    // add section end
    ...
    const editItem = useCallback((ev, rowData) => {
        ...
        modalPopup.custom({ header: "Update Role", className: "sdm-popup", visible: true });
    }, []);
    // update section start
                ...
                updateBtnsOptions: {
                    onClick: ({ selections, status }) => {
                        confirmDialog.custom({
                            message: "You are about to mass update the status of roles?",
